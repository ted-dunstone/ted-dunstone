...
EPT.fadeOutScene2 = function(sceneNameA, sceneNameB, context) {
  context.cameras.main.fadeOut(250);
  context.time.addEvent({
      delay: 250,
      callback: function() {
        context.scene.start(sceneNameA);
        context.scene.launch(sceneNameB);
        context.scene.setVisible(true, sceneNameB);
        //this.scene.remove(sceneNameB)
		    //this.scene.add(sceneNameB, HudOverlay, true, { x: 400, y: 300 });
        //context.scene.get(sceneNameB).afterinit();
        context.scene.get(sceneNameB).scene.bringToTop()
      },
      callbackScope: context
  });
};
